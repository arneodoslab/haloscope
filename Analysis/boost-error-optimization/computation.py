...
##-------------------------- DEFINING TRANSFER MATRIX FORMULATION BEGIN --------------------##
def G12(n2,n1):
...
def Bright(nList,dList,w,v):
    ...
    return sol
##-------------------------- DEFINING TRANSFER MATRIX FORMULATION END --------------------##


def absolute(complexVal):
    '''
    gives the amplitude of a complex number
    
    inputs:
        complexVal - a complex number
    
    returns:
        amplitude - amplitude of the complex number
    '''
...
def make_IndexList(n1,n2,nLayers,mirror=False):
    '''
    makes the list of refractive indices based on number of layers and refractive indices
    
    inputs:
        n1,n2 : values of the refractive indices of the materials
        nLayers : number of layers in the stack. Must be an even value
        mirror : indicates whether mirrored or not (bool). Note that mirror is the first layer
        
    returns:
        indList : array of refractive indices
    '''
...
def make_ThickList(d1,d2,nLayers):
    '''
    makes a list of thicknesses based on initial values and number of layers
    see make_chirplist function for a chirped list
    
    inputs:
        d1,d2 - thickness of layers in a bilayer stack
        nLayers - number of layers
    
    returns:
        thickList - array of thicknesses for each layer in the stack
    '''
...
def make_chirplist(d1_,d2_,n1_,n2_,nLayers_,chirping_):
    '''
    defines the thickness for stack layers with chirping. Note that chirp = 1 corresponds 
    to a stack with no chirping
    
    inputs:
        d1_, d2_ - thickness if it were a bilayer
        n1_,n2_ - refractive indices for the two materials
        nLayers_ - number of layers in the stack
        chirping_ - chirp factor to use
    
    returns:
        dList - array of thicknesses for each layer in a chirped stack
    '''
    x = Symbol('x')
...
def solution_rightwave(n1,n2,d1,nLayers,chirp=0,mirror=False,isTList=[]):
    '''
