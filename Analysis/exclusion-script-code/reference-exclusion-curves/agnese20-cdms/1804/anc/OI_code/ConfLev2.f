      Real*8 Function ConfLev(m,x,mu,icode)
...
C Tabulated information
      Integer*8 N,Nmeans,NCLs,NCMaxmu
      Parameter(N=50)
      Parameter(Nmeans=41)
      Parameter(NCLs=22)
      Parameter(NCMaxmu=70)
      Common/CLtab/CL,Mean,xcl,Nxcl,muval,CMaxval,meanlog
      Real*8 CL(NCLs),xcl(0:N,NCLs,Nmeans),mean(Nmeans),meanlog(Nmeans)
      Real*8 muval(NCMaxmu),CMaxval(NCMaxmu)
      Integer*8 Nxcl(0:N,Nmeans)
C
      Real*8 mulog,xcl2(NCLs+1),clxmu,GAMDIS,
     x CL2(NCLs+1)
      Integer*8 Nxcl2
      Logical first/.true./
