      Real*8 Function UpperLim(CL,If,N,FC,muB,FB,Iflag)
...
C  Exclude_low(2) and UpperLim is allowed.
      Implicit None
      Integer*8 N,If,Iflag,NMax,I,m,Niter,Istat,IflagOpt,MaxF,
     1 NMax1,N1,If1,NCalls,I1,I2,mmax,EndPoints,Hold1
...
C almost always get almost exactly the same answer.
      Parameter (Niter=10)
      Real*8 CL,FC(0:N+1),y_vs_CLf,CMaxinf,mu,mu0,f(0:NMax),y,y2,x,
     1 CMax,fmin(7)/.00,.01,.02,.05,.1,.2,.5/,eps/.0001/,Cdbg,
     2 fdebug,ydebug,mutmp,R,Topmu,Botmu,CL1,muB,FB(0:N+1),FC1,FB1,muB1,
     3 fmin1(7),mucand(3),TopNow,BotNow,FUPbot,FUPnow,Exclude_low,
     4 fhigh,fdiff
      Real*8 Fup
...
C time take UpperLim=mu.
      If(CL.lt.0.8D0 .or. CL.gt. 0.995D0) Then
         Write(6,*) CL,
...
C with muB>0.
     ...
     1                (muB/mu0)*(FB(I2+m+1)-FB(I2)))
            ...
            If(f(m).gt.fmin(If)) Then
               ...
               y2=y*y
               x=Dfloat(m)+ .5D0*(y2+Dsqrt(y2*(4.D0*Dfloat(m)+y2)))
               mutmp=x/f(m)
               If(mutmp.lt.mu) Then
                  mu=mutmp
C IflagOpt will have the status of the optimum interval
             ...
             EndIf
         EndDo
         If(dabs(mu0-mu)/mu .lt. eps .and. I.gt.1) Go to 100
         mu0=max(muB,mu)
...
C fails for mu>54.5.
      ...
      CL1=CL
      If1=If
      Topmu=dfloat(N)+4.D0*Dsqrt(DFloat(N))+5.D0
      Botmu=max(muB,dlog(1.D0/(1.D0-CL)))
      If( muB.gt.0.D0 .and. Fup(Botmu,Hold1) .gt.0.D0) Then
C It looks like UpperLim wants to be negative
      ...
      If(R.lt.0.D0 .or. Istat.gt.4) Iflag=Iflag+64
      If(mu.gt.2.5D0 .and. mu.lt.Max(12.,15.D0*CL-1.5D0)) Then
...
C 6th at 12.01; the table of CMax ends at 12 and covers CL=.8+ to near 1.
         BotNow=min(2.5D0,mu-1.5D0)
         TopNow=BotNow
         FUpbot=Fup(BotNow,Hold1)
         Do I=1,300
C         Do I=1,1000
            TopNow=TopNow+.01D0
C            TopNow=TopNow+.003D0
            If(TopNow.gt.12.D0) Go to 100
            FUpnow=Fup(TopNow,Hold1)
            If(FUpbot*FUpnow.lt.0.D0) Then
...
C 215     Format('f,C: ',10F8.5)
      ...
      Return
      End
      Real*8 Function Fup(x,I)
      Implicit None
      Integer*8 N1,NMax,I,If1,Istat,Icode,m,NCalls,I1,mmax
      Parameter (NMax=600000)
C      Real f(0:NMax),x,C,y,CMax,CL1,MeanMax,Cinf,CMaxinf,ConfLev,
      Real*8 f(0:NMax),x,C,y,CMax,CL1,MeanMax,Cinf,CMaxinf,ConfLevNew,
     ...
     1 FB1(0:NMax),muB1,fmin1,Cdbg(0:10),mmax
      ...
      Do m=0,N1
         If(muB1.ne.0.D0) Then
            f(m)=0.D0
            Do I1=0,N1-m
               f(m)=Max(f(m),(1.D0-(muB1/x))*(FC1(I1+m+1)-FC1(I1))+
