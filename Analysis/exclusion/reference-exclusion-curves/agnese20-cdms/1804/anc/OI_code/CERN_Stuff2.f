...
C
      ...
      ELSE
       ...
       IF(P .GT. HF) X=1-P
       X=DSQRT(-2.D0*DLOG(X))
       X=X-((7.47395D0*X+494.877D0)*X+1637.720D0)/
     1     (((X+117.9407D0)*X+908.401D0)*X+659.935D0)
       IF(P .LT. HF) X=-X
       S=X**2
       Z=C*(P-DFREQ(X))*DEXP(HF*S)
...
C        ACM Trans. Math. Software 5 (1979) 482-489.
     ...
     3 -0.0000201,-0.0000013, 0.0000011,-0.0000002/
      ...
      IF(A .GT. ALFA) THEN
       ...
       TERM=X*TERM/(A+K)
       SUM=SUM+TERM
       IF(DABS(TERM) .LE. EPS*SUM) GO TO 2
    1  CONTINUE
       GO TO 98
    2  HST=SUM*DEXP(A*ALX-X-ALOGAM(1+A))
      ELSEIF(X .GT. C1) THEN
       P=0
       S=1-A
       Q=(X+S)*(X-1-A)
       ...
       TERM=RHO*TERM
       SUM=SUM+TERM
       IF(DABS(TERM) .LE. EPS*SUM) GO TO 4
    3  CONTINUE
       GO TO 98
    4  HST=1-(A*SUM/(X+1-A))*DEXP(A*ALX-X-ALOGAM(1+A))
      ELSE
       IF(A .LT. HALF) THEN
        SUM=C(14)
        DO 12 K = 13,1,-1
   12   SUM=A*SUM+C(K)
        ...
        ELSE
         ...
         TERM=Y*TERM/K
         SUM=SUM+TERM
         IF(DABS(TERM) .LE. EPS1*SUM) GO TO 8
    7    CONTINUE
         GO TO 98
    8    U=GA-SUM*ALX
        ENDIF
       ELSE
        U=GAMMA(A)-DEXP(A*ALX)/A
       ENDIF
       P=A*X
       Q=A+1
       R=A+3
       ...
       TERM=-P*TERM/Q
       SUM=SUM+TERM
       IF(DABS(TERM) .LE. EPS1*SUM) GO TO 10
    9  CONTINUE
       GO TO 98
   10  HST=1-A*(U+SUM*DEXP((1+A)*ALX)/(1+A))/GAMMA(1+A)
      ENDIF
   99 GAMDIS=HST
      RETURN
  ...
  102 FORMAT('PROBLEMS WITH CONVERGENCE, X = ',E15.8,'  A = ',E15.8)
    ...
    3 U1=F1-F2
      ...
      ELSE
       ...
       U4=U3*U3-CC/CA
       IF(U4 .LT. 0) GO TO 1
       X0=-U3+SIGN(DSQRT(U4),X0+U3)
      END IF
      IF(X0 .LT. XA .OR. X0 .GT. XB) GO TO 1
 
      R=MIN(DABS(X0-X3),DABS(X0-X2))
      EE=EPS*(DABS(X0)+1)
      IF(R .GT. EE) THEN
       F1=F2
       X1=X2
...
C
    ...
    5  AQ=Q3(I)+X*AQ
      ...
      ELSE
       Y=1/X**2
       H=(X-HF)*DLOG(X)-X+C(4)+(C(1)+Y*(C(2)+Y*C(3)))/
     1                                        ((C(5)+Y)*X)
      ENDIF
      ALGAMA=H
      RETURN
